...
WHERE Numero_Vendite = (SELECT MAX(Numero_Vendite) FROM VenditePerModello);
    ...
    \vspace{0.2cm}
    \item \textbf{Query 2:} Selezionare i venditori che, in un intervallo temporale scelto dall'utente (es. 2025), hanno effettuato
    un numero di vendite superiore a una soglia minima (es. 3).
...
HAVING COUNT(*) >= 3;
    ...
    \vspace{0.1cm}
    \item \textbf{Query 3:} Per ciascun motore (identificato dal suo codice), determinare il ricambio che è stato utilizzato con maggiore frequenza complessiva in tutti gli interventi effettuati sui veicoli che montano quel motore.
...
La colonna \textbf{Data\_Acquisto} è messa per prima per restringere immediatamente il numero di righe da esaminare.
La colonna \textbf{Venditore} come secondo attributo aiuta il \textbf{GROUP BY} perché le righe di ciascun venditore risultano già ordinate all'interno dell'intervallo.

Un'alternativa sarebbe stata un indice su (Venditore, Data\_Acquisto), che, tuttavia, sarebbe meno efficace perché il filtro sulla data non potrebbe essere sfruttato come primo livello di selezione. La scelta ricade quindi su (Data\_Acquisto, Venditore).
